...
parser.add_argument("--gpu", type=str, default='1', help="gpu", dest='gpu')
...
def main():
    ...
    # create dataset
    dev   = CoNLLDataset(config.filename_dev, config.elmofile_dev, config.processing_word,
                         config.processing_postags, config.generate_anchor,
                         config.max_iter)
    train = CoNLLDataset(config.filename_train, config.elmofile_train, config.processing_word,
                         config.processing_postags, config.generate_anchor,
                         config.max_iter)
    test  = CoNLLDataset(config.filename_test, config.elmofile_test, config.processing_word,
                         config.processing_postags, config.generate_anchor,
                         config.max_iter)
